...
async function isSportsEventFinal(claim: ClaimOnChain, evidenceText: string): Promise<boolean> {
  ...
  try {
    const text = await throttledLLM(prompt, { maxTokens: 64, jsonOnly: true, model: pickGeminiModel("oracle"), jsonSchema: { type: "object", properties: { final: { type: "boolean" } }, required: ["final"] } });
...
async function settle(claim: ClaimOnChain): Promise<boolean> {
  ...
  if (claim.category.toLowerCase() === "sports") {
    ...
    if (!pastGrace &&!(await isSportsEventFinal(claim, evidence.text))) {
      console.log(`[settle] Claim #${claim.id}: match not final yet — deferring.`);
      return false;
    }
  }
  if (evidence.payment) {
    const paid = unitsToUsdc(BigInt(evidence.payment.priceUnits));
    console.log(`[settle] Paid ${paid.toFixed(6)} USDC for evidence`);
  }
  ...
  if (COUNCIL_SETTLEMENT) {
    ...
    if (council && COUNCIL_SELF_RESOLVING) {
      ...
      councilCommitment = { tally: council.tally, qChain: council.qHistory?? [], referenceQ: Number(referenceQ.toFixed(4)), scores: council.votes.filter(v=>v.probability!==undefined).map(v=>Number((v.score??0).toFixed(4))) };
  ...
  if (bonusVotes && COUNCIL_BONUS_ATOMIC > 0n) {
    try {
      ...
      if (!isConfirmedCouncilSettlement(confirmed, verdictToSide(verdict.verdict), evidenceHash, Boolean(settled.pending))) {
        console.warn(`[settle] Bonus withheld: not confirmed`);
      } else {
        const receipts = await payCouncilBonuses({ votes: bonusVotes, poolAtomic: COUNCIL_BONUS_ATOMIC, payerWallet: ORACLE, claimId: claim.id, contractId: CONTRACT_ID, settlementTxHash: settled.txHash });
        for (const r of receipts) console.log(`[settle] Bonus ${formatAtomicUsdc(r.amountAtomic)} USDC to ${r.slug}: ${r.status}`);
      }
...
async function challengeIfMispriced(claim: ClaimOnChain): Promise<void> {
  ...
  // ── ISSUE #111: BOUND 1 — malformed, cancelled, paused, duplicate ──
  const validation = riskManager.validateClaim(claim);
  if (!validation.ok) {
    console.log(`[challenge] Skip #${claim?.id}: ${validation.reason} — ${validation.detail}`);
    if (validation.reason === "malformed" || validation.reason === "dependency") riskManager.recordFailure(validation.reason);
    if (claim) riskManager.recordEvaluated(claim.id);
    return;
  }
  ...
  // ── BOUND 2 — circuit breaker / cooldown ──
  if (riskManager.isInCooldown()) {
    console.log(`[risk] Cooldown active — stats:`, riskManager.getStats());
    return;
  }

  // ── BOUND 3 — funded-state safety ──
  let balances;
  try {
    balances = await readAgentBalances(ORACLE_ADDR);
  } catch {
    riskManager.recordFailure("dependency");
    return;
  }
  ...
  // ── BOUND 4 — dependency failure & stale ──
  if (evidence.fetcher === "none") {
    console.log(`[challenge] No evidence — dependency failure`);
    riskManager.recordFailure("dependency");
    riskManager.recordEvaluated(claim.id);
    return;
  }
  if (isEvidenceStale(evidence.fetchedAt)) {
    console.log(`[challenge] Stale evidence #${claim.id} — skip`);
    riskManager.recordEvaluated(claim.id);
    return;
  }
  ...
  try {
    ...
    console.warn(`[challenge] LLM dependency failure #${claim.id}:`, e.message);
    riskManager.recordFailure("dependency");
    return;
  }
  ...
  if (verdict.verdict!== "CHALLENGERS_WIN" || verdict.confidence < CHALLENGE_CONFIDENCE) {
    console.log(`[challenge] Not confident enough — skip`);
    riskManager.recordEvaluated(claim.id);
    return;
  }
  ...
  let kellyStake = Math.max(CHALLENGE_STAKE_USDC, Math.min(bankroll * kelly, bankroll * 0.1));
  kellyStake = Math.min(kellyStake, riskManager.config.maxStakePerClaimUsdc);
  const stakeUsdc = Math.round(kellyStake * 100) / 100;

  // ── BOUND 5 — exposure limits ──
  const can = riskManager.canChallenge(stakeUsdc);
  if (!can.ok) {
    console.log(`[challenge] Blocked #${claim.id}: ${can.reason} — ${can.detail}`, riskManager.getStats());
    riskManager.recordEvaluated(claim.id);
    return;
  }

  console.log(`[challenge] Kelly: ${(kelly*100).toFixed(1)}% → ${stakeUsdc} USDC | stats before:`, riskManager.getStats());
  try {
    ...
    console.log(`[challenge] ✓ Staked ${stakeUsdc} USDC — ${staked.explorerUrl?? staked.txHash} | exposure now ${riskManager.getStats().exposure}`);
  } catch (e:any) {
    console.error(`[challenge] Tx failed #${claim.id}:`, e.message);
    riskManager.recordFailure("dependency");
    // do not mark challenged — allow retry after cooldown
  }
}
...
async function poll(): Promise<void> {
  const now = Math.floor(Date.now()/1000);
  let total: number;
  try { total = await getClaimCount(); } catch (err) { console.warn("[oracle] Failed claim count", err); riskManager.recordFailure("dependency"); return; }
  ...
  for (let id=1; id<=total; id++) {
    ...
    if (!claim) continue;
    if (claim.state === "active" && claim.deadline <= now) { expiredActive.push(claim); continue; }
    try {
      if ((claim.state === "open" || claim.state === "active") && claim.deadline > now) {
        const before = riskManager.getStats().concurrent;
        await challengeIfMispriced(claim);
        if (riskManager.getStats().concurrent > before) challenged.push(id);
      }
    } catch (err) { console.error(`[oracle] Error on claim ${id}:`, err); }
  }
  ...
  for (let i=0;i<expiredActive.length;i++) {
    ...
    try {
      ...
      riskManager.recordSettled(claim.total_challenger_stake);
      if (i < expiredActive.length-1 && SETTLEMENT_DELAY_MS>0) {
        console.log(`[oracle] Cooling down ${(SETTLEMENT_DELAY_MS/60000).toFixed(1)} min...`);
        await new Promise(r=>setTimeout(r,SETTLEMENT_DELAY_MS));
      }
    } catch (err) { console.error(`[oracle] Error settling claim ${claim.id}:`, err); }
  }
  const summary = [settled.length?`Settled: [${settled.join(", ")}]`:null, challenged.length?`Challenged: [${challenged.join(", ")}]`:null].filter(Boolean).join(" | ");
...
async function main(): Promise<void> {
  ...
  if (!balances.exists) throw new Error(`oracle account ${ORACLE_ADDR} does not exist — run: npm run agents:fund`);
  console.log("═══════════════════════════════════════════════");
  console.log(" Mimir Oracle Agent — BOUNDED RISK (Issue #111)");
  console.log(` Contract : ${CONTRACT_ID}`);
  console.log(` Oracle : ${ORACLE_ADDR}`);
  console.log(` Bankroll : ${balances.usdc===null?"no trustline":`${balances.usdc.toFixed(4)} USDC`}`);
  console.log(` Network : Stellar ${STELLAR_NETWORK}`);
  console.log(` LLM : ${activeLLMProvider()} / ${activeLLMModel()}`);
  console.log(` Risk : daily=${DEFAULT_RISK_CONFIG.maxDailyChallengeUsdc} USDC, exposure=${DEFAULT_RISK_CONFIG.maxTotalExposureUsdc} USDC, concurrent=${DEFAULT_RISK_CONFIG.maxConcurrentChallenges}, per-claim max=${DEFAULT_RISK_CONFIG.maxStakePerClaimUsdc} USDC`);
